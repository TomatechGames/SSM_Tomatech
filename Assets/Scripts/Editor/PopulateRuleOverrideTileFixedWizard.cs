...
public class PopulateRuleOverideTileFixedWizard : ScriptableWizard
{
    [MenuItem("CONTEXT/RuleOverrideTile/Populate From Sprite Sheet FIXED")]
    static void MenuOption(MenuCommand menuCommand)
    {
        CreateWizard(menuCommand.context as RuleOverrideTile);
    }
    [MenuItem("CONTEXT/RuleOverrideTile/Populate From Sprite Sheet FIXED", true)]
    ...
    {
        var wizard = DisplayWizard<PopulateRuleOverideTileFixedWizard>("Populate Override FIXED", "Populate");
    ...
    public static void CloneWizard(PopulateRuleOverideTileFixedWizard oldWizard)
    {
        var wizard = DisplayWizard<PopulateRuleOverideTileFixedWizard>("Populate Override FIXED", "Populate");
        ...
        try
        {
            ...
            foreach (RuleTile.TilingRule rule in m_tileset.m_Tile.m_TilingRules)
            {
                foreach (Sprite originalSprite in rule.m_Sprites)
                {
                    ...
                    Sprite matchingOverrideSprite = overrideSprites.First(sprite => sprite.name == spriteName);
